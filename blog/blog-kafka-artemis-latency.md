...
Recently I asked colleague why we use multiple queue implementations instead of 
just putting Kafka everywhere. The answer was something that I, frankly, would 
not expected and it was "because Kafka is designed for throughput and not 
...
Suppose we have microservice architecture application where service A
must query service B before it can respond to customer, e.g. Navigation
service providing some path between 2 locations and depending on Traffic
service which collects data on current traffic congestion.
...
If you are unfamiliar with JAX-RS (let those servlets go, it's 2018),
[here](https://jersey.github.io/documentation/latest/jaxrs-resources.html)
is a little description.

This resource accepts `GET` requests and having that it sleeps for 1
second and then returns body containing invocation number. That number
will be useful later to visualize what happened during test runs.
...
Let's emulate our calls to our slow service in test suite. Following
handy Dropwizard rule will create a fresh embedded server for each test:
...
and routine execution duration. Ignore `@Suspendable` for now, we'll come
back to it later.
...
First 2 long tasks start immediately, since we have 2 threads available
in pool. However other subsequent tasks cannot start and are put into
thread pool's queue waiting to be picked up. This way  blocking calls
stop execution and our application just burns CPU cycles doing nothing
(but showing 100% utilization).
...
fibers will finish before test returns. In real-life scenarios you would
not need to do that, because long running tasks idiomatically must be
solved via `@AsyncResponse`. Codewise, thread example looks like fiber
one less `Future` collection code.
...
All invocations started at approximately same time and finished within
2 s. Int his case threads are not blocked by waiting and we could
actually manage huge amount of requests with little threads (generally
same size as number of CPU cores).
...
to avoid CPU being stalled when waiting for network or file IO.
...
nested (see [callback hell](http://callbackhell.com/)).
...
sequential logic of execution, while still avoiding blocking.
...
threads is a viable solution.

All new libraries must be justified to be
used and no performance optimizations must be done without defined
performance requirements or before application logic is working as per
requirements. However, once you need performance, using coroutines is
crucial as it fits first optimization level (IO-bound optimizations).

#### Does this all come for free?

Unfortunately no. This quick blog post does not highlight some
important details needed in order for example code to work. Since Java
does not have language support for coroutines, some joggling is needed.

First, in order for coroutines to work compiled Java bytecode must
...
[here](https://docs.oracle.com/javase/7/docs/api/java/lang/instrument/package-summary.html)).
Here is very brief explanation of what it does. During class loading
registered agents will scan loaded class for methods that have
`@Suspended` annotation or `throws SuspendedException` in signature.
Those will be modified to allow saving stack into memory and suspend
execution during specific calls.
...
4. if you are using existing library to perform IO it may not have
methods correctly annotated. If it's not the case you may need to create
your own modification of library to be used together with fibers. In
this example I used jersey async client library which already has
modified fiber-friendly version:
```
